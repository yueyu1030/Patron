...
for train_label in 2304 2560 3072; do #  8 16 32 64
for seed in 0  ; do
for train_seed in 87 100 128 ; do # 128 0 21 42 87 100
for label_per_class in 0 ; do
for model_type in roberta-base ; do
# margin-km kmeans kmeansw coreset 
for al_method_first in random-1 random-2 random-3  ; do # kmeans_refine1_r0.01 kmeans_refine1 kmeans_refine1_r0.005   kmeansw tpc kmeans entropy cal bertkm coreset bald badge kmeansw tpc
...
train_cmd="CUDA_VISIBLE_DEVICES=${gpu} python3 main_unsupervised.py --do_train --do_eval --task=${task} \
	--train_file=train_32.json --dev_file=valid_64.json --test_file=test.json \
