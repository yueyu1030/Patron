...
for seed in 0  ; do
for train_seed in 200 300 400 500 ; do # 128 0 21 42 87 100
for label_per_class in 0 ; do
for model_type in roberta-base ; do
for al_method_first in  random-1 random-2 random-3   ; do #kmeans_refine1 entropy cal bertkm coreset bald badge kmeansw tpc
for a in 0.3 ; do  #0.3 0.1
for b in 1 ; do #0.1 0.5 0 1 5 0.5_0.5 0.2_0.5 0.01_0.1 0.1_0.1 0_0.1 5_0.1 0.5_0.2 
for c in 0.1; do # 0.3
# random-1 random-2 random-3 kmeans kmeansw coreset margin-km
...
train_cmd="CUDA_VISIBLE_DEVICES=${gpu} python3 main_unsupervised.py --do_train --do_eval --task=${task} \
	--train_file=train_32.json --dev_file=valid_32.json --test_file=test.json \
